<template>
  <div class="city_container">
    <scroll-view style="height: 100vh;" :scroll-into-view="scrollLetter" enable-back-to-top scroll-y>
      <div class="right-letter">
        <span>热门</span>
        <span v-for="letter in letters" :key="letter" @click="scrollLetter = letter">{{letter}}</span>
      </div>
      <div class="location">
      ...
      <div class="hot-city">
        ...
        <div class="letter" v-for="(item, key) in cityData" :key="key" :id="item.letter">
          {{item.letter}}
          <ul class="city-list">
            <li v-for="(city, index) in item.citys" :key="index" @click="chooseCity(city)">{{city}}</li>
          </ul>
        </div>
      </div>
    </scroll-view>
